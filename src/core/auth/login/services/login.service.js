 ...
 */
export async function authenticateWithPassword({ email, senha }) {
  const usuario = await getUserEmailAndPassword({ email });
...
export async function verifyActivationCode({ email, verificationCode }) {
  ...
  const registro = await getLatestActivationCode({
    usuarioId: usuario.id,
  });
  const now = new Date();
  ...
  if (!codigoValido) {
    await incrementActivationAttempts({
